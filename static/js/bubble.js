...
(function (root, factory) {
  ...
  var pi2 = Math.PI * 2;
  /**
   * Bubble Chart implementation using {@link d4js.org|d4js}
   *
   * @class BubbleChart
   * @example
   ...
   */
  d4.svg.BubbleChart = function (settings) {
    var self = this;
    ...
    self.moveToCentral(d4.select(".node"));
  };


  $.extend(d4.svg.BubbleChart.prototype, {
    ...
    setup: function () {
      ...
      self.svg = d4.select(options.container).append("svg")
      ...
      var fnColor = d4.scale.category20();
      node.append("circle")
 ...
 moveToCentral: function (node) {
      ...
      var self = this;
      var toCentralPoint = d4.svg.transform()
    ...
    moveToReflection: function (node, swapped) {
      var self = this;
      var toReflectionPoint = d4.svg.transform()
        .translate(function (d) {
    ...
    registerClickEvent: function (node) {
      ...
      node.style("cursor", "pointer").on("click", function (d) {
        self.clickedNode = d4.select(this);
  ...
  MicroPlugin.mixin(d4.svg.BubbleChart);

  return d4.svg.BubbleChart;
}));
/**
 ...
 * }
 * ```
 * @param {function} [color=d4.scale.category20] - Function should return a string used to fill bubbles <br/>ex:
 * ```js
